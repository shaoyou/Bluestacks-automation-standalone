...
import { LogActions, PageHeading, Toggle } from "../components/layout";
...
export function ScriptsPage(props: SharedProps) {
  ...
  const capture = async (mode: CaptureMode) => {
    ...
    try {
      const image = await window.bsManager.screenshot({ adbPath: props.settings.adbPath, hdcPath: props.settings.hdcPath }, editorDevice);
  ...
  const debug = () => {
    ...
    const args = [`${props.runtime.root}/adb_bot.py`, "--plan", `${props.runtime.plansDir}/${props.activePlan}`, "--adb", props.settings.adbPath, "--hdc", props.settings.hdcPath];
...
export function RunnerPage(props: SharedProps & { runnerId?: string; initialPlan?: string; standalone?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (selection.device && props.devices.includes(selection.device)) return;
    updateSelection({ device: props.devices[0] ?? "" });
  }, [props.devices, selection.device]);
