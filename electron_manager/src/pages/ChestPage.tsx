...
export function ChestPage(props: SharedProps) {
  ...
  const [selectedLabelItemIds, setSelectedLabelItemIds] = useState<string[]>([]);
  const [deletingLabelItems, setDeletingLabelItems] = useState(false);
  ...
  const loadDay = async (day: string) => {
    ...
    const syncedRecords = events
      ...
      .map((event) => ({ before_path: String(event.screenshot_path ?? ""), before_saved_at: String(event.captured_at ?? ""), source_name: event.source_name, remote_only: true, item_event: event }));
  ...
  const toggleLabelItemSelection = (itemId: string) => {
    setSelectedLabelItemIds((current) => current.includes(itemId) ? current.filter((id) => id !== itemId) : [...current, itemId]);
  };

  const deleteSelectedCatalogItems = () => {
    const selected = selectedLabelItemIds.filter((itemId) => unlabeledItems.some((item) => item.itemId === itemId));
    if (!selected.length || deletingLabelItems) return;
    if (!window.confirm(`删除选中的 ${selected.length} 个物品图鉴？历史开箱记录不会删除。`)) return;
    setDeletingLabelItems(true);
    void (async () => {
      for (const itemId of selected) await window.bsManager.chestDeleteItem(itemId);
      await refreshHistory(true);
      openLabelManager();
    })().catch((error) => props.setNotice(`删除物品失败: ${String(error)}`)).finally(() => setDeletingLabelItems(false));
  };
  ...
  const openCalibration = (record: Record<string, unknown>, event: Record<string, unknown> | undefined) => {
    ...
    const fallbackSource = selectedSource();
    const recordSourceId = String(record.source_id ?? event?.source_id ?? fallbackSource?.id ?? "boss_jinjia");
    const recordSourceName = String(record.source_name ?? event?.source_name ?? fallbackSource?.name ?? "金甲");
  ...
  const saveCalibration = () => {
    ...
    const corrections: ChestItemCorrection[] = calibrationItems.map((item, index) => {
      ...
      const itemName = ((calibrationNameDrafts[index + 1] ?? "").trim() || String(item.item_name ?? "").trim() || "待标注物品");
  ...
  return <div className="page">
    ...
    {showLabelModal ? <div className="chest-label-backdrop" role="dialog" aria-modal="true" aria-labelledby="chest-label-title"><section className="chest-label-modal"><header><div><strong id="chest-label-title">物品标注</strong><span>已设置权重的物品优先显示，其次是待标注物品。</span></div><div className="chest-label-header-actions"><button className="button secondary" disabled={deletingLabelItems} onClick={() => { setEditingLabels((current) => !current); setSelectedLabelItemIds([]); }}>{editingLabels ? "完成" : "编辑"}</button><button className="icon-button" title="关闭" onClick={closeLabelManager}><X size={17} /></button></div></header><div className="chest-label-list">{unlabeledItems.length ? unlabeledItems.map((item) => <div key={item.itemId} className={`chest-label-row ${editingLabels ? "selecting" : ""} ${item.labeled ? "labeled" : ""}`}>{editingLabels ? <label className="chest-label-select" title={`选择 ${item.name}`}><input type="checkbox" checked={selectedLabelItemIds.includes(item.itemId)} onChange={() => toggleLabelItemSelection(item.itemId)} /></label> : null}<img src={labelImages[item.itemId] ?? ""} alt={item.name} /><div><code>{item.itemId}</code><small>{item.labeled ? `已标注：${item.name}` : "待标注"}，出现 {item.occurrences} 次</small></div><input autoComplete="off" placeholder="物品名称" value={labelDrafts[item.itemId] ?? ""} onChange={(event) => setLabelDrafts((current) => ({ ...current, [item.itemId]: event.target.value }))} /><input inputMode="decimal" placeholder="权重" value={weightDrafts[item.itemId] ?? ""} onChange={(event) => setWeightDrafts((current) => ({ ...current, [item.itemId]: event.target.value.replace(/[^0-9.]/g, "") }))} /><button className="button primary" disabled={!labelDrafts[item.itemId]?.trim() || !!savingItemId || deletingLabelItems} onClick={() => saveLabel(item)}>{savingItemId === item.itemId ? "保存中" : "保存"}</button></div>) : <p className="empty-note padded-note">尚无已识别的物品。</p>}</div><footer><span>{editingLabels ? `已选择 ${selectedLabelItemIds.length} 个，${unlabeledItems.length} 个物品` : `${unlabeledItems.filter((item) => !item.labeled).length} 个待标注，${unlabeledItems.length} 个物品`}</span><div className="chest-label-footer-actions">{editingLabels ? <button className="button danger" disabled={!selectedLabelItemIds.length || deletingLabelItems} onClick={deleteSelectedCatalogItems}><Trash2 size={16} />{deletingLabelItems ? "删除中" : "删除"}</button> : null}<button className="button secondary" onClick={closeLabelManager}>关闭</button></div></footer></section></div> : null}
